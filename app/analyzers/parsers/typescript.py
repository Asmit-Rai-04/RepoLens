...
class TypeScriptParser(TreeSitterParserBase):
    ...
    def _build_parser(self):
        ...
        return Parser(Language(ts_typescript.language_typescript()))

    def _extract(self, path: Path, source: bytes, tree) -> SourceFile:
        source_file = self._extract_common(path, source, tree)
        for node in descendants_of_type(tree.root_node, "interface_declaration", "type_alias_declaration", "enum_declaration"):
            ...
            if name:
                ...
                source_file.symbols.append(symbol_from_node(node, name, kind))
        return source_file

    def _extract_common(self, path: Path, source: bytes, tree) -> SourceFile:
        symbols = []
        imports = []
        inheritance = []
        ...
        for node in descendants_of_type(
            tree.root_node, "function_declaration", "function", "class_declaration", "method_definition",
            "variable_declarator", "import_statement", "call_expression",
        ):
            if node.type == "function_declaration":
                ...
                name = name_from_field(node, source)
                if name:
                    symbols.append(symbol_from_node(node, name, "method", nearest_class(node)))
                ...
                match = re.search(r"from\s+['\"]([^'\"]+)['\"]", text) or re.search(r"import\s+['\"]([^'\"]+)['\"]", text)
                if match:
                    named = re.search(r"\{([^}]*)\}", text, re.DOTALL)
                    names = [p.strip().split(" as ")[0].strip() for p in named.group(1).split(",") if p.strip()] if named else []
                    imports.append(Import(
                        module=match.group(1), kind="import", imported_names=names,
                        line_start=node.start_point[0] + 1, line_end=node.end_point[0] + 1,
                    ))
                ...
                if function is not None and node_text(function, source) == "require" and arguments is not None:
                    match = re.search(r"['\"]([^'\"]+)['\"]", node_text(arguments, source))
                    if match:
                        imports.append(Import(module=match.group(1), kind="require", line_start=node.start_point[0] + 1, line_end=node.end_point[0] + 1))
